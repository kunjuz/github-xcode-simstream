...
static const CGFloat kTabWidth = 80;
...
- (void)layoutSubviews
{
	...
	frame.size.width = CGRectGetWidth(self.view.frame);
    frame.size.height = [self window_height];//CGRectGetHeight(self.view.frame) - (topLayoutGuide + CGRectGetHeight(self.tabsView.frame)) - (self.tabBarController.tabBar.hidden ? 0 : CGRectGetHeight(self.tabBarController.tabBar.frame));
...
- (void)setActiveTabIndex:(NSUInteger)activeTabIndex
{
	...
	[self.tabsView scrollRectToVisible:frame animated:self.defaultSetupDone];
}
...
- (void)defaultSetup
{
	...
	// Select starting tab
	NSUInteger index = self.activeContentIndex;//self.startFromSecondTab ? 1 : 0;
	[self selectTabAtIndex:index didSwipe:NO];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
	...
	if (self.shouldAnimateIndicator == ViewPagerIndicatorAnimationWhileScrolling) {
